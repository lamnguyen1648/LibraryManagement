...
namespace LibraryManagement.Forms.QuanLyNhaXuatBan
{
    public partial class ThemNxbForm : Form
    {
        ...
        private void ResolveTableName()
        {
            ...
            foreach (var cand in new[] { "NXB", "NhaXuatBan" })
            {
                try
                {
                    using var cmd = new SqlCommand($"SELECT TOP(1) * FROM {cand}", conn);
                    ...
                    _tableName = cand;
                    foreach (var n in new[] { "TenNXB", "Ten", "TenNhaXuatBan" })
                        if (dt.Columns.Contains(n)) { _nameColumn = n; break; }
                    if (_nameColumn == null)
                        _nameColumn = dt.Columns.Cast<DataColumn>()
...
ORDER BY c.column_id;";
            ...
            // Always require the name column (TenNXB)
        ...
        // ---------- Regex map ----------
        // Returns a regex (if any) and an error message for the given column name.
        private static bool TryGetRegex(string col, out Regex regex, out string message)
        {
            ...
            // Số điện thoại (VN): bắt đầu bằng 0 hoặc +84, tiếp theo 9–10 chữ số
            ...
            // Mã số thuế Việt Nam: 10 hoặc 13 chữ số
            ...
            // Địa chỉ: tối thiểu 5 ký tự; cho phép chữ, số, khoảng trắng và , . / - # ( )
        ...
        // Validate one field; set/remove ErrorProvider message
        ...
        private void UpdateAddEnabled()
        {
            ...
            // Check all fields (required + regex)
            foreach (var name in _controlsByColumn.Keys)
            {
                if (!ValidateField(name)) { allOk = false; break; }
            }
        ...
        private void DoInsert()
        {
            try
            {
                ...
                // Final gate: ensure everything valid
                UpdateAddEnabled();
            ...
            {
                MessageBox.Show("Không thể thêm NXB: " + ex.Message, "Lỗi",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            "tennxb" or "ten" or "tennhaxuatban" => "Tên Nhà Xuất Bản",
            "diachi"        => "Địa chỉ",
            "email" or "mail" => "Email",
