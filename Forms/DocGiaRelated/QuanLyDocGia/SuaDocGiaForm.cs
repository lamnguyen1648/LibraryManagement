...
{
    public partial class SuaDocGiaForm : Form
    {
...
ORDER BY c.column_id;";
            ...
            while (rd.Read())
            {
                ...
                if (fallback == null && (name.Equals("DG_ID", StringComparison.OrdinalIgnoreCase) ||
                                         name.Equals("ID", StringComparison.OrdinalIgnoreCase)))
                    fallback = name;
            }
            _idColumnName ??= fallback ?? "DG_ID";
        }
...
ORDER BY c.column_id;";
        ...
        private void BindRow()
        {
            ...
            foreach (var col in _cols)
            {
                ...
                if (c is DateTimePicker dtp && value is DateTime d) dtp.Value = d;
                else if (c is CheckBox chk) chk.Checked = value != null && Convert.ToBoolean(value);
                else if (c is TextBox tb) tb.Text = value?.ToString() ?? "";
            }
        }
